...
- 자식 글머리 항목을 형제 사이로 드래그 이동할 때 직접 부모 목록을 기준으로 삭제·삽입 범위를 계산하고, 핸들/여백 좌표에서도 대상 자식 항목을 정확히 잡도록 수정.
...
- 갤러리 편집 팝업에서 블록 높이를 설정하고 공개 페이지·복제 블록에도 같은 높이를 동기화하도록 개선.
- 갤러리 이미지를 설정 높이 안에서 원본 비율로 모두 보이도록 표시 방식을 개선.

### Fixed

- 공개 드롭다운에서 같은 워크스페이스에 별도로 게시된 페이지를 해당 공개 URL로 안전하게 이동하도록 수정하고, 게시 해제·삭제·DB 행·타 워크스페이스 항목은 계속 숨기도록 보강.
- YouTube 임베드 iframe에 필요한 `compute-pressure` 권한만 위임해 공개 페이지 콘솔의 권한 정책 경고를 제거.
- 페이지 본문 번역 시 일반 버튼 블록의 라벨도 번역하면서 링크·색상·DB 연결 속성은 유지하도록 수정.
...
- AppSync가 이중 직렬화한 공유 블록 JSON도 정상 해제해, 서버에 저장된 갤러리 이미지와 드롭다운 항목을 빈 데이터로 잘못 해석하던 문제 수정.
- 새로고침 시 오래된 빈 공유 블록 seed가 페이지에 저장된 갤러리 스냅샷을 가리지 않도록 복원 우선순위를 보강.
- 공유 블록 저장 경쟁 상황에서도 최신 서버 데이터를 페이지 인라인 스냅샷에 반영하고 즉시 동기화하도록 수정.

## [5.6.78] - 2026-07-19

### Changed

- 드롭다운 메뉴를 내용 너비에 맞게 표시하고 드래그 핸들 메뉴에서 좌측·중앙·우측 정렬을 선택할 수 있도록 개선.
- 드롭다운 메뉴 설명을 번역 용도에 한정하지 않는 일반적인 공유 페이지 메뉴 표현으로 변경.

### Fixed

- 드롭다운 메뉴와 갤러리 저장 직후 페이지 문서를 즉시 동기화해 새로고침 시 블럭이 사라질 수 있던 문제 수정.
- 드롭다운의 현재 페이지 항목도 클릭 시 연결된 페이지를 다시 열도록 수정.

## [5.6.77] - 2026-07-18

### Added

- 여러 페이지에서 같은 데이터를 공유하는 드롭다운 메뉴·롤링 갤러리 블록과 편집 팝업을 추가.
- 게시된 일반 페이지 제목줄에 웹 게시 설정을 즉시 여는 지구본 버튼을 추가.

### Changed

- 공개 페이지의 공유 블록을 서버 최신 데이터로 안전하게 치환하고, 갤러리 자산 사용처와 공개 범위를 함께 검증하도록 강화.
- 공개 페이지의 목차 버튼을 텍스트 없이 아이콘만 표시하도록 정리.
...
- **이모지 shortcode**: `:체크`, `:핀`처럼 콜론 뒤 키워드를 입력하고 Space를 누르면 이모지로 자동 치환.
- **이모지 단축어 목록**: `/이모지` 피커에 단축어 탭을 추가해 지원 키워드를 확인하고 바로 삽입.
- **커스텀 아이콘 자산 보호**: 커스텀 아이콘으로 등록된 자산을 미사용 자산 정리 대상에서 제외.
- **탭 컨텍스트 메뉴**: 상단 탭 우클릭 메뉴에 링크복사·탭복제를 추가하고, 탭 닫기 버튼을 탭 모서리에 표시.

### Changed

- **데이터베이스 탭 클릭 피드백**: DB 탭 클릭 영역을 사이드바 페이지와 같은 포인터·press scale 동작으로 정리.
- **인라인 코드 스타일**: 백틱 장식을 숨기고 회색 박스·빨간 텍스트로 표시.
- **내부 링크 새 탭 열기**: 페이지 멘션·블럭 링크·버튼 링크를 Ctrl/Cmd+클릭하면 새 상단 탭으로 열림.

### Fixed

- **DB 항목 페이지 폭 토글**: 상단 제목·속성 패널이 페이지 전체 너비 설정을 따르도록 수정.
...
- **워크스페이스 전환 체감 개선**: LC 스케줄러 데이터를 일반 워크스페이스 전환 경로에서 분리하고, 캐시 기반 화면 전환·댓글 초기 렌더 흐름을 보정.
- **데이터베이스 UX 보정**: 인라인 DB 삭제 동작 제한, 팝업 텍스트 크기, 리스트뷰 날짜 표시, 표시 설정/연결 DB 검색 흐름을 정리.
- **LC 스케줄러 타임라인 안정화**: 좌측 구성원 컬럼과 타임라인 스크롤 구조를 단일 스크롤 기준으로 교체하고, 줌/툴팁/특이사항 카드 조작을 일정 카드와 맞춤.
- **박스 선택/드래그 감도 보정**: 페이지 본문 텍스트 선택과 박스 드래그 충돌을 줄이고, 스크롤 민감도를 조정.
