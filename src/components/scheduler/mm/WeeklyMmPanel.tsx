import { AlertTriangle, ChevronLeft, ChevronRight, ChevronsDown, Lock, Save } from "lucide-react";
import { useEffect, useState } from "react";
import { LC_SCHEDULER_WORKSPACE_ID } from "../../../lib/scheduler/scope";
import { buildWeeklyMmSuggestion, toMmScheduleSource } from "../../../lib/scheduler/mm/mmSuggestion";
import { getDefaultMmWeek, getMmWeekLabel, shiftMmWeek } from "../../../lib/scheduler/mm/weekUtils";
...
export function WeeklyMmPanel() {
  ...
  const targetMember = members.find((member) => member.memberId === targetMemberId) ?? me ?? null;
  const effectiveTargetMemberId = targetMember?.memberId ?? null;

  useEffect(() => {
    if (!me) return;
    if (!targetMemberId || (!admin && targetMemberId !== me.memberId)) {
      setTargetMemberId(me.memberId);
    }
  }, [admin, me, targetMemberId]);
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-5 right-5 z-[620] rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg hover:bg-blue-700"
      >
        주간 MM 열기
      </button>
    );
  ...
  return (
    <section className="fixed bottom-5 right-5 z-[620] w-[420px] rounded-lg border border-zinc-200 bg-white shadow-2xl dark:border-zinc-700 dark:bg-zinc-900">
      <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-zinc-700">
        <div>
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => setWeekStart((v) => shiftMmWeek(v, -1))} className="rounded p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800" aria-label="이전 주">
              <ChevronLeft size={16} />
            </button>
            <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
              {getMmWeekLabel(weekStart)}
            </h3>
            <button type="button" onClick={() => setWeekStart((v) => shiftMmWeek(v, 1))} className="rounded p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800" aria-label="다음 주">
              <ChevronRight size={16} />
            </button>
          </div>
          <span className={`mt-1 inline-flex rounded px-2 py-0.5 text-xs ${statusTone}`}>
            {statusLabel}
          </span>
        </div>
        <button
          ...
          className="inline-flex items-center gap-1 rounded border border-zinc-200 px-2 py-1 text-xs text-zinc-500 hover:bg-zinc-50 dark:border-zinc-700 dark:hover:bg-zinc-800"
        >
      ...
      <div className="max-h-[70vh] space-y-3 overflow-y-auto p-4">
        {admin && (
          <select
            value={effectiveTargetMemberId ?? ""}
            onChange={(event) => setTargetMemberId(event.target.value)}
            className="w-full rounded border border-zinc-200 bg-white px-2 py-1.5 text-sm text-zinc-900 focus:outline-none focus:ring-2 focus:ring-blue-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"
          >
            {members.map((member) => (
