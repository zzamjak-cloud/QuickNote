...
import { notionImportDebug } from "./debugLog";
...
function diagMem(label: string) {
  ...
  if (m) {
    notionImportDebug(`[ZIP진단] ${label} | 힙 ${(m.usedJSHeapSize / 1048576).toFixed(0)}MB / 한도 ${(m.jsHeapSizeLimit / 1048576).toFixed(0)}MB`);
  } else {
    notionImportDebug(`[ZIP진단] ${label}`);
  }
}
...
async function parseNotionZipInput(input: ZipInput): Promise<NotionZipPreview> {
  ...
  notionImportDebug(`[ZIP진단] 파싱 시작 — 입력 ${(inputSize / 1048576).toFixed(1)}MB`);
  diagMem("파싱 시작");
  ...
  while (queue.length > 0) {
    ...
    notionImportDebug(`[ZIP진단] JSZip.loadAsync 호출 — basePath="${current.basePath}" ${(currentSize / 1048576).toFixed(1)}MB`);
    ...
    notionImportDebug(`[ZIP진단] 항목 수: ${currentFiles.length}개`);
  ...
  diagMem("파싱 완료");
  notionImportDebug(`[ZIP진단] 파싱 완료 — 페이지 ${pages.length}개(MD:${markdownEntries.length} HTML:${htmlEntries.length}) 에셋 ${assetEntries.length}개`);
  return {
