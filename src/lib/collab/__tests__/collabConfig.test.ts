...
describe("collabConfig", () => {
  ...
  it("buildCollabWsUrl 은 token·pageId 를 쿼리스트링으로 인코딩하고 room 에 epoch 솔트(기본 v3)를 싣는다", () => {
    (import.meta.env as Record<string, unknown>).VITE_COLLAB_WS_URL = "wss://x/dev";
    const url = buildCollabWsUrl("p1", "tok en/+=");
    expect(url).toContain("pageId=v3%3Ap1");
  ...
  it("buildDbCollabWsUrl 은 db:<epoch>: prefix room 을 pageId 파라미터에 싣는다(기본 v3)", () => {
    (import.meta.env as Record<string, unknown>).VITE_COLLAB_WS_URL = "wss://x/dev";
    const url = buildDbCollabWsUrl("db-1", "tok");
    expect(url).toContain("pageId=db%3Av3%3Adb-1");
