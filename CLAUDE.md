...
## UI 규약: 팝업/드롭다운 화면 클리핑 금지 (필수)

**원칙**: 퀵노트 내 모든 팝업, 드롭다운, 팝오버, 메뉴 리스트는 절대로 뷰포트 바깥으로 잘려서는 안 된다. 화면 크기·스크롤 위치·트리거 위치에 따라 위치를 동적으로 보정해야 한다.

**구현 표준**:
- 셀/버튼 기준 팝오버는 반드시 `useAnchoredPopover` (`src/hooks/useAnchoredPopover.ts`) 를 사용한다. 이 hook 은 다음을 자동 처리한다:
  - 1차 위치: 트리거 아래.
  - 팝업이 화면 아래로 넘치면 트리거 위로 자동 플립.
  - 좌·우 모두 화면 안쪽으로 클램프 (8px 패딩).
  - 윈도우 resize/scroll/팝업 콘텐츠 크기 변화에 ResizeObserver 로 재계산.
- `useAnchoredPopover` 가 맞지 않는 케이스(예: 자유 부유 패널, 부착 기준이 마우스 좌표)에는 동일한 클램프·플립 로직을 직접 구현하고, 가능하면 공용 유틸로 분리한다.
- 새로 만드는 메뉴/모달이 화면 바깥으로 나갈 가능성이 있으면 반드시 사전에 시뮬레이션(작은 화면, 화면 가장자리 근처 트리거)으로 확인한다.
