...
| 플로우차트(도형·화살표·동기화·히스토리·전체보기·링크) | `flowchart/overview.md` |
| 플로우차트 도형 찌그러짐·화살표 안 보임·정렬·미리보기 높이 | `flowchart/overview.md` (회귀 표) |
| 공유 드롭다운 메뉴·롤링 갤러리·공개 페이지 번역 이동 | `blocks/shared-blocks.md` |
...
| 데이터 손실 방지·진단·payload 가드 | `infra/data-safety.md` |
| 기술 스택·공통 util 단일 출처 | `infra/tech-stack.md` |
| 관측성(reportNonFatal beacon)·dev/live 캐시 격리 | `infra/observability.md` |
| 번들 코드분할(lazy IconPicker/markdown/lowlight)·manualChunks | `infra/bundle.md` |
| 서버 디스패치(v5-resolvers resolver map)·새 resolver 추가 | `infra/server-dispatch.md` |
| PWA 설치·Service Worker·업데이트 배너·오프라인 정합 | `pwa/overview.md` |
| 모바일 반응형(셸 drawer·z-index·터치 click·DB 카드·블록 액션 시트·컬럼 스택) | `mobile/overview.md` |
| 모바일 키보드 뜸·블록 링크 터치 무반응 | `mobile/overview.md` (stopEvent·contentEditable 패턴) |
| 인라인 DB 제목 숨기기·헤더 컬러 | `mobile/overview.md` → `database/DatabaseColumnSettingsButton` |
...
| 파일 | 내용 |
|------|------|
| `overview.md` | PWA 설치·SW·업데이트 UX·캐시 정책·오프라인 정합·불변식 |

### mobile/
| 파일 | 내용 |
|------|------|
| `overview.md` | 반응형 전략·z-index 규칙·터치 click 패턴·화면별 적응·인라인 DB 표시설정 |
...
| 파일 | 내용 |
|------|------|
| `tech-stack.md` | 기술 스택·공통 util 단일 출처·설정 파일 |
| `ai.md` | AI 제공사·모델 화이트리스트·Gemini 마이그레이션·검증 |
| `deploy.md` | 배포 체크리스트 |
| `version-sync.md` | 버전 동기화 규칙(`npm run bump`) |
| `data-safety.md` | 데이터 손실 방지·payload 가드·데드코드 12건 |
| `observability.md` | reportNonFatal(ring buffer+beacon)·dev/live Tauri 식별자 격리 |
| `bundle.md` | 코드분할(IconPicker/markdown/lowlight lazy)·manualChunks 룰 |
| `server-dispatch.md` | v5-resolvers switch→resolver map·새 resolver 추가 지점 |
| `cost-optimization-deploy.md` | 서버 비용 최적화 배포·마이그레이션 런북(TTL·GSI·BatchGet·증분) |

### flowchart/
| 파일 | 내용 |
|------|------|
| `overview.md` | 플로우차트 — 도형 렌더 규칙, 엣지, 미리보기, flowchartId 공유·서버 동기화, 링크, 자동저장·전체보기·버전 히스토리, 서버 스택, 회귀 표 |
...
| `shared-blocks.md` | 페이지 복사 후 동기화되는 드롭다운 메뉴·롤링 갤러리, 공개 hydrate·자산 규칙 |
