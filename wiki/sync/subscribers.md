...
## 2026-06-15 사이드바 이동 동기화 사고 기록

증상:
- 웹/데스크톱 간 페이지 생성·삭제·사이드바 이동이 실시간으로 반영되지 않거나, 첫 이동만 반영되고 이후 이동이 멈춤.
- Notion import 페이지처럼 본문이 큰 페이지에서 사이드바 drag/drop 위치 변경이 특히 잘 누락됨.
- 새로고침하면 일부 위치가 뒤늦게 보이는 경우가 있어 서버 저장 자체와 실시간 fan-out 문제를 구분해야 했다.

확정 원인:
- AppSync subscription WebSocket 인증에 `authToken` 옵션을 사용해 데스크톱/Tauri 경로에서 handshake 인증이 안정적으로 붙지 않았다.
- `ON_PAGE_CHANGED` 구독은 서버가 meta-only payload 를 발행하는데 클라이언트가 full `GqlPageSchema` 로 검증해 `doc` 없는 이벤트를 버릴 수 있었다.
- 사이드바 이동은 `parentId`/`order` 만 바뀌는데도 `movePage`가 full `enqueueUpsertPage(p)`를 호출했다. 큰 Notion import 페이지에서는 `MAX_UPSERT_PAGE_PAYLOAD_BYTES` 제한에 걸려 upsert enqueue/flush 가 누락되고, 그 결과 실시간 이벤트도 발행되지 않았다.

수정:
- subscription 시작 시 `authMode: "none"` + `additionalHeaders { Authorization: idToken }` 로 WebSocket handshake 인증을 명시한다.
- `onPageChanged` 수신은 `GqlPageMetaSchema` 로 검증하고, channel `next` 내부 예외는 try/catch 로 막아 한 이벤트 실패가 구독 전체를 죽이지 않게 한다.
- 사이드바 이동은 `metaOnly: true` 옵션으로 enqueue 한다. 클라이언트는 placeholder `doc`를 보내되 `UPSERT_PAGE_META` mutation 으로 meta field 만 응답받고, 서버는 기존 placeholder 보존 가드로 의미 있는 기존 본문을 덮어쓰지 않는다.
- `normalizePageInput`에는 `fullPageDatabaseId`를 포함해 meta-only upsert 경로에서도 사이드바 hidden 상태가 유실되지 않게 한다.

검증:
- 개발 Tauri + 웹앱에서 사이드바 이동 실시간 동기화 성공 확인.
- `npm run lint`, `npm run typecheck`, `npm run test:run`, `npm run build` 통과.
- infra `build:resolvers`, `npm test`, live `cdk diff/deploy` 통과.
