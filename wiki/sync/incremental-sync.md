...
## 증분 모드 좀비 페이지 대사 (2026-07-03)

영구삭제(hard delete)는 델타에 tombstone 이 없어 증분만 도는 PC 에 유령 페이지가 남는다
(실사례: CAT 복제 후 삭제→휴지통 영구삭제, 다른 PC 에 페이지 잔존 + collab WS 거절 반복).
`fetchApplyWorkspaceRemoteSnapshot` 델타 경로가 **앱 세션당 워크스페이스별 1회**, 전체
페이지 메타 ID 목록(`fetchPageMetasByWorkspace`, 권위)과 대사해 서버에 없는 좀비를
`reconcileWorkspacePagesFullSnapshot`(outbox pending 보호 포함)로 정리한다.
전체 목록 조회 실패 시 prune 은 위험하므로 건너뛰고 다음 증분에서 재시도.
페이지 진입 시 자기치유(page-content-load.md 참고)와 상호 보완 — 이쪽은 진입 없이도 낫는다.
2026-07-03 v5.6.7 라이브 배포로 실사례 해결 확인(워크스페이스 진입만으로 유령 소거).
