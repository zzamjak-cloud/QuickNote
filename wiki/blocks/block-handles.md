...
**특수 규칙**:
- `listItem`/`taskItem`: **own row(마커 + 첫 텍스트 라인)** 위에 마우스가 있을 때만 핸들 표시 (`pointInsideListOwnRow`). 항목 안에 중첩된 블록(코드 블록·이미지·표 등)은 own row 가 아니므로 각자 자기 드래그 핸들을 가진다 — 이 블록들 위에서는 listItem 핸들이 잡히지 않고 내부 블록 핸들이 표시된다.
...
listItem 의 own row 안에 마우스가 있는지 판별. own row = 리스트 마커 + 첫 텍스트 블록(문단/헤딩, taskItem 은 본문 div 안 첫 문단)의 세로 범위. 그 아래 중첩 블록(코드 블록·이미지·중첩 리스트 등)은 own row 에서 제외 → 중첩 블록이 자기 핸들을 갖는다. (전체 자식 rect 를 own row 로 보면 `considerListItemHandleFromStack` 이 depth+100 으로 내부 블록 핸들을 강탈하던 회귀가 있었다.)
