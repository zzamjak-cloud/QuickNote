...
- 갤러리 본문 이미지는 `.ProseMirror img` 전역 기본값(`height:auto`, margin)의 영향을 받지 않도록
  `qn-shared-gallery`/`qn-shared-gallery-image` 전용 CSS로 격리한다. 미리보기 dialog는
  Portal로 `.ProseMirror` 밖에 렌더되므로, 미리보기만 정상이고 롤링 영역만 비는 회귀는 이 격리 규칙을 먼저 확인한다.
