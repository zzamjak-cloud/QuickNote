...
## 삭제 행 유령 방지 (tombstone 필터)
삭제된 행 페이지가 리스트에 계속 보이고/클릭 시 "항목 페이지를 불러오지 못했습니다"가 뜨던 버그의 원인:
- `deletePage`는 로컬 prune(`removePagesFromAllIndexes`) + softDelete enqueue + `markLocallyDeletedEntity` 까지 한다.
- 그러나 백그라운드 row-index 워밍업(`externalProtectedDatabaseLoad` → `fetchDatabaseRowIndexBatch` → `upsertRows`)이 **서버가 여전히 돌려주는 삭제 행**을 다시 snapshot 에 넣어 유령 재유입.
- 검색/멘션 후보엔 `createLocalDeletionFilter`(localDeleteGuards) 가 적용돼 있었지만 **DB 행 후보(`useProcessedRows` 의 `rowPageOrder`)엔 빠져 있었다.**
- 수정: `rowPageOrder` 조립 시 tombstone 필터로 로컬 삭제 행을 제외(`useProcessedRows.ts`). `createDatabaseRowSourcesSelector` 는 `rowPageOrder` 만 순회하므로 여기가 단일 chokepoint.

### cached-only 행은 deleteRow no-op 였음 (중복 유령 행이 "삭제 불가" 였던 진짜 원인)
- `gqlPageToDatabaseRowIndexEntry` 는 `page.deletedAt` 있으면 제외한다. 즉 row-index 에 계속 들어오는 유령 행은 **서버에 `deletedAt` 이 안 찍힌**(soft-delete 누락) 행이다.
- `deleteRow`(databaseStore.ts)는 `pageBefore = pages[pageId]` 기준으로 동작 → row-index 에만 있고 pageStore 에 페이지가 없는 **cached-only 행은 `deletePage` 가 no-op** → tombstone·서버 `softDeletePage` enqueue·row-index prune 이 전부 누락. 결과적으로 **삭제 버튼이 무동작**이고 서버가 영원히 같은 행을 돌려줘 중복까지 쌓였다.
- 수정: `deleteRow` 에서 `!pageBefore` 면 row-index 스냅샷에서 workspaceId 를 찾아 `markLocallyDeletedEntity` + `removePagesFromAllIndexes` + `enqueueAsync("softDeletePage")` 를 직접 호출. softDelete 가 서버 `deletedAt` 을 찍으면 다음 row-index fetch 에서 제외된다.
- 한계: tombstone 은 7일(영구 30일) TTL 이지만 위 수정으로 서버 soft-delete 가 확정되므로 근본 해결. (store 파일 변경 → dev 완전 재시작 필요)
