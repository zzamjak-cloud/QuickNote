...
3. `nextToken`이 있으면 `fetchDatabaseRowIndexBatch()`로 남은 row index만 백그라운드에서 순차 로드한다.
4. `useDatabaseRowIndexStore.upsertRows()`가 snapshot을 갱신하고 로컬 캐시에 저장한다.
5. `useProcessedRows()`는 `bundle.rowPageOrder`와 row index pageId를 합쳐 후보군을 만들고, `databaseRowSources`가 row index fallback을 제공한다.
6. 필터·정렬·검색은 전체 row index 후보군 기준으로 즉시 계산된다.
