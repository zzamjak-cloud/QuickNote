...
- 템플릿 적용으로 만드는 일반 행은 기본값과 템플릿 셀을 `createRowPage`의 초기 `dbCells`로 넘기고, 같은 값을 DB 협업 Y.Doc의 `rows`에 즉시 시드한다. 최종 페이지 업서트도 `includeCells: true`로 보내 페이지 생성과 속성 durable mirror가 분리되지 않게 한다.
...
- full page에 `_qn_isTemplate = "1"` 마커가 있지만 같은 DB의 `dbTemplates`에 `pageId`가 없으면 registry orphan으로 간주한다. full-content 적용 시점과 DB snapshot 적용 시점 모두에서 누락 항목만 자동 복원하고 독립 템플릿 버전으로 서버에 영속한다. meta-only 이벤트에는 `dbCells`가 없으므로 마커 복원 근거로 사용하지 않는다.
