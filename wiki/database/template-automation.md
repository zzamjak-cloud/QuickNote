...
- 협업 DB의 Y.Doc 구조에는 templates가 없다. 템플릿 변경은 reconcile된 Y.Doc의 최신 컬럼·프리셋 구조와 templates를 함께 서버에 보내고, 이후 materialize 업서트도 현재 templates를 포함해 outbox dedupe가 값을 유실하지 않게 한다.
...
- 구버전 클라이언트·기존 레코드에 `templatesUpdatedAt`이 없으면 `updatedAt`을 폴백으로 사용한다. 구조 이벤트와 템플릿 이벤트의 도착 순서가 바뀌어도 클라이언트는 두 버전을 독립 비교한다.
