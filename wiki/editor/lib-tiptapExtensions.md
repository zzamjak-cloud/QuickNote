...
| `youtubeBlock.tsx` | `YoutubeBlock` | YouTube 임베드 블록. nocookie 모드. iframe에 필요한 권한만 위임 |
...
| `buttonBlock.tsx` | `ButtonBlock` | 클릭 가능한 버튼 블록. 페이지 본문 번역 시 일반 버튼의 `label` 속성도 번역 |
| `databaseBlock.ts` | `DatabaseBlock` | 인라인 데이터베이스 블록 |
| `flowchartBlock.tsx` | `FlowchartBlock` | `flowchartId`를 공유하는 서버 동기 플로우차트 atom 블록 |
| `sharedBlocks.tsx` | `DropdownMenuBlock`, `GalleryBlock` | `sharedBlockId`를 공유하는 번역 메뉴·롤링 갤러리 atom 블록. 공개 변환 규칙은 [shared-blocks](../blocks/shared-blocks.md) 참고 |
...
| `toggle.tsx` | `Toggle`, `ToggleHeader`, `ToggleContent` | 토글 블록 3종. 에디터 렌더는 React NodeView(`div.toggle-block`)라 `indent`/`backgroundColor`/`blockTextColor` attr 을 NodeViewWrapper 의 `data-indent`/`data-bg-color`/`data-text-color` 로 직접 렌더해야 한다. 새 토글 제목은 빈 `toggleHeader` + CSS placeholder(`토글 제목`)이고, 글머리/목록 선택 상태에서 토글로 타입 변경하면 첫 텍스트 블록의 인라인 내용을 토글 제목으로 승격한다 |
...
| `deleteCurrentBlock.ts` | `DeleteCurrentBlock` | 현재 블록 삭제 단축키. 콜아웃 내부 글머리/토글 제거 시 콜아웃 wrapper가 아니라 내부 블록만 제거한다 |
| `insertBeforeBlock.ts` | `InsertBeforeBlock` | 블록 앞에 새 단락 삽입 단축키 |
| `indentation.ts` | `Indentation` | Tab 들여쓰기 extension. 선택 위치에서 가장 바깥 들여쓰기 대상 조상 블록을 직접 갱신하므로 `toggleHeader` 안에서 Tab/Shift-Tab 시 `toggle` wrapper 의 `data-indent` 가 바뀐다 |
...
| `textStyleWithoutSplit.ts` | `TextStyleWithoutSplit` | `TextStyle`의 `keepOnSplit`을 끈 확장. Enter로 새 제목/본문 줄을 만들 때 인라인 컬러 mark를 이어받지 않음 |
...
- **YouTube iframe 권한**: 웹/공개 페이지의 직접 iframe과 Tauri용 `public/yt-embed.js` 내부 iframe 모두 `allow`에 `compute-pressure`를 포함한다. YouTube 플레이어에만 기능을 위임하며 전역 와일드카드 정책으로 확장하지 않는다.
- **버튼 블록 번역**: `translateInPlace.ts`는 일반 `buttonBlock.attrs.label`을 번역하되 `href`·`databaseId`·`color` 등 나머지 속성은 보존한다. `databaseId`만 사용하는 DB 전용 버튼은 DB 제목에서 라벨을 다시 파생하므로 번역 대상에서 제외한다.
...
- **인라인 텍스트 색 Enter 분리**: 에디터에는 기본 `TextStyle` 대신 `TextStyleWithoutSplit`을 등록한다.
  `keepOnSplit: false`는 `textStyle(color)`만 새 줄에서 제거하며 bold/italic 등 별도 mark는 유지한다.
