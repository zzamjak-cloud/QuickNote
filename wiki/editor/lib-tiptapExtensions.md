...
| 파일 | Export | 설명 |
|------|--------|------|
| `mention.tsx` | `MentionExtension` | `@멤버`·`@페이지`·`@DB` 통합 인라인 멘션 **단일 노드** (`mentionKind` 분기). 페이지 멘션도 이 확장이 렌더(class `page-mention`). **멘션 렌더·PM mousedown(멤버/DB) 수정은 이 파일.** 페이지 **클릭 이동**은 `pageMentionClick.ts` → [navigation/overview.md](../navigation/overview.md) |
...
| 파일 | Export | 설명 |
|------|--------|------|
| `markdownCodeBlockPreview.tsx` | `CodeBlockWithMarkdownPreview`, `CodeBlockLowlightWithMarkdownPreview` | 마크다운 언어 codeBlock — 미리보기/소스 탭. 미리보기 영역 `select-text` 로 드래그 복사 가능 |
...
- **마크다운 붙여넣기**: `Ctrl/Cmd+Shift+V` → `useEditorProps` → `pasteMarkdownAsDocContent` (`lib/editor/pasteMarkdownAsDoc.ts`) — 클립보드 마크다운을 `notionMarkdownToDoc` 으로 변환해 본문 블록으로 삽입(코드블록 아님).
