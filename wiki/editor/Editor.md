...
- **텍스트 선택/스크롤 복원 보정**: `useEditorProps`의 drag-scroll dampener 는 본문 자동 스크롤만 제한한다. 우측 overlay scrollbar 입력은 mousedown 시점과 scroll 시점 모두에서 보정 로직을 해제한다. `pageScrollMemory` 는 우측 네이티브 스크롤바 hit 영역의 `pointerdown`/`mousedown`을 사용자 스크롤로 마킹하고, Chrome처럼 네이티브 scrollbar drag가 scroll 이벤트만 내는 경우에도 복원 target 에서 벗어난 scroll 을 사용자 개입으로 보고 복원 루프를 중단한다.
